...
if missing_cols:
    ...
    aggregated.columns = ['player', 'nation', 'appearances', 'teams', 'position']

    # Reorder columns
    aggregated = aggregated[['player', 'nation', 'teams', 'position', 'appearances']]

    # Sort by appearances (descending) for better readability
    aggregated = aggregated.sort_values(by='appearances', ascending=False)
    ...
    # Save to JSON
    aggregated.to_json('players_pl.json', orient='records', lines=True, indent=4)